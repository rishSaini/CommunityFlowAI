...
// Fields that were recently auto-filled flash a green ring
...
export default function PartnerIntakeForm({ form, onChange, flashFields = [], onSubmit, onReset }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    setSubmitted(true);
    setTimeout(() => { setSubmitted(false); onReset(); }, 3500);
  };
  ...
  const flash = (field: FlashField) =>
    flashFields.includes(field)
      ...
      : "border-slate-200 bg-slate-50/50 focus:border-indigo-400 focus:bg-white";
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 animate-fade-in">
        ...
        <p className="text-slate-500 text-sm text-center max-w-xs leading-relaxed">
          Our AI is triaging your request. Check the dashboard to see your priority score.
        </p>
        <div className="mt-5 flex items-center gap-2 bg-indigo-50 text-indigo-700 px-4 py-2 rounded-full text-sm font-medium border border-indigo-200">
          <Sparkles size={13} /> AI triage in progress…
        </div>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          <Building2 size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 z-10" />
          <input
            type="text" value={form.name}
            onChange={(e) => set("name", e.target.value)}
            placeholder="e.g., Hillcrest High School"
            className={`w-full pl-9 pr-4 py-2.5 rounded-xl border text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all duration-300 ${flash("name")}`}
            required
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <CalendarDays size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 z-10" />
          <input
            type="date" value={form.eventDate}
            onChange={(e) => set("eventDate", e.target.value)}
            className={`w-full pl-9 pr-4 py-2.5 rounded-xl border text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all duration-300 ${flash("eventDate")}`}
            required
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <div className="relative">
            <MapPin size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 z-10" />
            <input
              type="text" value={form.city}
              onChange={(e) => set("city", e.target.value)}
              placeholder="Midvale"
              className={`w-full pl-9 pr-3 py-2.5 rounded-xl border text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all duration-300 ${flash("city")}`}
              required
            />
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            <select
              ...
              className={`w-full pl-3 pr-8 py-2.5 rounded-xl border text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all duration-300 appearance-none ${flash("county")}`}
              required
            >
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-9 pr-3 py-2.5 rounded-xl border text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all duration-300 ${flash("zipCode")}`}
              required
            />
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-9 pr-3 py-2.5 rounded-xl border text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all duration-300 ${flash("attendeeCount")}`}
              required
            />
          </div>
        </div>
      ...
      <div>
        ...
        <div className={`grid grid-cols-2 gap-2 p-2 rounded-xl transition-all duration-300 ${flashFields.includes("needs") ? "bg-emerald-50/60 ring-2 ring-emerald-200 rounded-xl" : ""}`}>
      ...
      >
        {submitting
          ? <><Loader2 size={15} className="animate-spin" />AI Processing…</>
