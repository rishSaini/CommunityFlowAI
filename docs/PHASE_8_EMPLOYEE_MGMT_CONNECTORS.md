...
| 2026-03-21 | Claude | **Partner Direct Messaging.** New `partner_messages` table, `messages.py` router (3 endpoints: channels, get messages, send message). 3 partner accounts (jennifer/carlos/lisa) with role="partner". 16 seeded demo messages across 3 conversations. Frontend: `ChatChannel` (reusable chat UI with polling), `StaffMessagesView` (split-pane inbox), `PartnerDashboard` (request list + chat). LoginPage updated with partner sign-in option. Staff gets "Messages" tab, partners get "My Requests" tab. Admin sees all channels. |
